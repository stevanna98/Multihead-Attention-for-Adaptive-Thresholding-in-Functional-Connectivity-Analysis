...
class MyDataset(Dataset):
    def __init__(self, data, labels, percentile=50):
        self.matrices = data
        self.y = labels
        self.percentile = percentile  # Soglia da usare per il thresholding
    ...
    def __getitem__(self, idx):
        ...
        # Creazione della matrice thresholdizzata
        thresholded_matrix = self.thresholding(matrix, self.percentile)

        # Conversione in tensori PyTorch
        matrix = torch.tensor(matrix, dtype=torch.float32)
        thresholded_matrix = torch.tensor(thresholded_matrix, dtype=torch.float32)
        label = torch.tensor(label, dtype=torch.long)

        return matrix, thresholded_matrix, label
