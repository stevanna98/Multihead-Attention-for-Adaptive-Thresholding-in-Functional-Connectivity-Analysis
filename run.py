...
def main():
    ...
    parser.add_argument('--dim_hidden_', type=int, default=64, help='Hidden dimension')
    parser.add_argument('--num_heads', type=int, default=16, help='Number of heads')
    parser.add_argument('--sparser_num_heads', type=int, default=4, help='Sparser number of heads')
    ...
    parser.add_argument('--gnn_intermediate_dim', type=int, default=64, help='GNN intermediate dimension')
    parser.add_argument('--gnn_output_node_dim', type=int, default=64, help='GNN output node dimension')
    ...
    parser.add_argument('--sparsity_target', type=float, default=0.7, help='Sparsity target')
    parser.add_argument('--sparsity_lambda', type=float, default=100, help='Sparsity lambda')
    ...
    # Create dataset
    dataset = MyDataset(data=matrices, labels=labels, percentile=args.threshold) if args.model_type == 'network' else GraphDataset(func_matrices=matrices, labels=labels, threshold=args.threshold)
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(matrices, labels)) if args.model_type == 'network' else enumerate(skf.split(dataset, dataset.y)):
        ...
        if args.model_type == 'network':
            model = Model(
                ...
                lambda_sparsity=args.sparsity_lambda,
        ...
        # Callbacks
        monitor = 'val_f1'
        early_stopping = EarlyStopping(monitor=monitor, patience=20, mode='max')
